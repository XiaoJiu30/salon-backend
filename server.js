...
const Category = require('../backend/models/Category'); // Adjust path as needed
...
const paymentBookingRoute = require('../backend/routes/paymentBookingRoute');
...
app.get('/api/employees/service', async (req, res) => {
  ...
  try {
    ...
    `;
    const result = await pool.query(query, [service_id, branch_id]);
    res.json(result.rows);
  } catch (error) {
    console.error('Error fetching employees:', error);
    res.status(500).json({ error: 'Failed to fetch employees' });
